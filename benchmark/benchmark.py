...
def _mount_mp(cfg: DictConfig, mount_dir :str) -> None:
    """
    Mount an S3 bucket using Mountpoint, using the configuration to apply Mountpoint arguments.
    """
    MOUNT_BINARY = "/home/ec2-user/mountpoint-s3/target/release/mount-s3"
...
def run_experiment(cfg: DictConfig) -> None:
    ...
    try:
        _mount_mp(cfg, mount_dir)
        start_time, end_time = _run_fio(cfg, mount_dir)
        metadata = Metadata(start_time=start_time, end_time=end_time)
    finally:
